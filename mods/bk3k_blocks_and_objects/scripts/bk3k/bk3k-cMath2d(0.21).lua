...
local wall        --accepts a table of coordinates representing end points of wall links
                  --returns a table of all block(not float) coordinates for every point connecting/including those points
                    --This in turn can be used for world.placeMaterial, setMaterialSpaces(), etc
                  --You could also dump the return to a log so that you can add it properly to objects
                    --without needing to recalculate each time
local link        --used by wall, builds all block points between and including two coordinate points
                    --could be used on it's own for some purposes
local walls       --accepts a series of unconnected walls(made of links)
                  --and builds a unified spaces table from them
local zone        --returns a table of all spaces within a box, not including the corners/edges
local zones       --uses points build boxes to define zones, calls zone for each one, merge tables and return result
...
size = function(t1, t2)
  return {math.abs(t1[1] - t2[1]) + 1, math.abs(t1[2] - t2[2]) + 1}
end

abs = function(t)
  return {math.abs(t[1]), math.abs(t[2])}
end
...
max = function(t1, t2)
  return {math.max(t1[1], t2[1]), math.max(t1[2], t2[2])}
end

min = function(t1, t2)
  return {math.min(t1[1], t2[1]), math.min(t1[2], t2[2])}
end
...
boxCollisions = function(...)
  ...
  local nArgs = #t + 1 --offset + 1 so I can use < instead of <= which I bet is cheaper
...
wall = function(coT)  --isn't currently working!  Returns the first point only!

  local pointCount = #coT
  if not (pointCount > 1) then  --can't connect fewer than 2 points!
    return coT
  end
  ...
  local retTable
  local tempTables = { }
  retTable, s = link(coT[1], coT[2])
    --'s' = the size of the first link and 'retTable' = the contents

  while p < pointCount do
    local i = s + 1   --first index to be used in retTable
    local j = 2       --first point from tempTable would already be in retTable so skip it
    local t           --temporty size holder
    tempTable, t = link(coT[p], coT[p + 1])

    p = p + 1
    s = s + t         --would include an extra incriment because we'll skip the first coordinate in tempTable
                      --and that's fine because we can < instead of <=

    while i < s  do
      retTable[i] = tempTable[j]
      i = i + 1
      j = j + 1
    end

    s = s - 1         --deincrement for thrown away coordinate
  end

  if match(coT[1], coT[pointCount]) then  --destination returned to origin, throw away duplicate coordinate
    retTable[s] = nil
    s = s - 1
  end
...
link = function(t1, t2)
  local relativeX = t2[1] - t1[1]   --destination - source
  local relativeY = t2[2] - t1[2]
  local currentX = t1[1]
  local currentY = t1[2]
  local retTable = {t1}
  local i = 1
  ...
  while (currentX ~= t2[1]) or (currentY ~= t2[2]) do
    i = i + 1

    if currentX ~= t2[1] then
      currentX = currentX + incX
    end
    if currentY ~= t2[2] then
      currentY = currentY + incY
    end

    retTable[i] = {currentX, currentY}
  end
...
walls = function(waT)
  ...
  for k, v in ipairs(waT) do
    walls[k], s[k] = wall(v)
  end

  local retTable = {}
  for i, w in ipairs(walls) do
    for _, coord in ipairs(w) do
      table.insert(retTable, coord)
    end
  end
  --just merged coordinates from all nested tables
  return retTable
...
zone = function(r)
  ...
  local currentX = lowX + 1
  local currentY = lowY
  
  while currentY < highY do
    while currentX < highX do
      retTable[i] = {currentX, currentY}
      currentX = currentX + 1
    end
    currentY = currentY + 1
...
local ignore = {
[[
  local retTable = walls[1]
  local total = s[1]
  local currentIndex = total + 1

  for i, w in ipairs(walls) do
    if not i == 1 then
      local wallIndex = 1
      total = total + s[i]
      while currentIndex < (total + 1) do  ----rethink this
        retTable[currentIndex] = w[wallIndex]
