...
function extraInit()
  ...
  if (#storage.connectedIDs < 1) then
    local objectSize = cMath.size(storage.corners.upperLeft, storage.corners.lowerRight)
    --local tableHolder = cMath.encircle(storage.doorPosition, objectSize)
    local tableHolder = cMath.encircle(storage.center, objectSize)
    storage.touchPoints = storage.touchPoints or {
      tableHolder[3],
      tableHolder[7]
